...
void MacrodynGraphicsItem::drawThirdAxis()
{
    QPair<QLineF, QColor> pair(QLineF(0, 0,200,200), QColor(0,0,255));
...
//    if (!m_image->isNull())
//    {
//        QPair<QLineF, QColor> pair(QLineF(lmargin, hig+upmargin, wid+lmargin+4, upmargin), QColor(255,0,0));
//        m_listLock.lockForWrite();
//        m_lines << pair;
//        m_listLock.unlock();
...
//        QPolygon pointsX;
//        pointsX << QPoint(-5, 5);         /* coords for arrow on x-axis */
//        pointsX << QPoint(5, -5);
//        pointsX << QPoint(5, 5);
...
//        painter->setPen(AXISCOLOR);
//        painter->setBrush(AXISCOLOR);
//        painter->drawLine(lmargin, hig+upmargin, wid+lmargin+4, upmargin); /* X */
//        pointsX.translate(wid+lmargin+10, hig+upmargin+10);/*set starting point for x-arrow*/
//        /*draw x-arrow */
//        painter->drawPolygon(pointsX);
...
void MacrodynGraphicsItem::setBigPoint(qreal v, qreal w, const QColor& color, int size)
{
    ...
    if (!m_image->isNull())
    {
        ...
        path.addEllipse(::transform(m_axis, m_image->size(), pair.first), ((double)size)/2.0, ((double)size)/2.0);
...
void MacrodynGraphicsItem::savePdfInterior(const QString& path)
{
//    QFileDialog dialog;
...
//    system("/usr/local/texlive/2011/bin/x86_64-darwin/pdflatex --interaction=nonstopmode --output-directory=/Users/jtiwi/Desktop/LatexTest /Users/jtiwi/Desktop/LatexTest/lt.tex");
    ...
    printer.setOutputFileName(path);
    QPainter painter(&printer);
    double xscale = printer.pageRect().width()/double(wid);
    double yscale = printer.pageRect().height()/double(hig);
    double scale = 1.01*qMin(xscale, yscale);
////    painter.translate(printer.paperRect().x() + printer.pageRect().width()/2,
////                       printer.paperRect().y() + printer.pageRect().height()/2);
    painter.scale(scale, scale);
    painter.translate(-44, -34);
    QFont font = painter.font();
    font.setPixelSize(24);
    painter.setFont(font);
    pdfTextMarginx = -21;
