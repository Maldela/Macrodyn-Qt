...
void finanzmarkt_Jan::loadParamset(QTextStream& inFile)
{ 	
...
void finanzmarkt_Jan::iteration(const qint64& t)
{ 	
...
qreal* finanzmarkt_Jan::setLabels(const QString& label)
{
    if (label == "eta_F") return(&eta_F);
    if (label == "eta_C") return(&eta_C);
    if (label == "r") return(&r);
...
void finanzmarkt_Jan::saveParamset(QTextStream& outFile)
{	
...
void finanzmarkt_Jan::receiveParameters(const QList<qreal>&)
