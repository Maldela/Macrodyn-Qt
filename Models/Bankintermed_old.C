...
S=(1-nu)*e*s;
	...
	y=b/a*pow((e+I),a);
//	df_reg=alpha*df_min+(1-alpha)*df_max;
...
real Bankintermed::I_G_Fktn(real)
{
	I_G = (df_max - df)/(e + I);
...
//
// Author:			Michael Meyer
// Last modified:	Wed Feb  7 15:38:31 MET 2001
// By:				Michael Meyer
//
///////////////////////////////////////////////////////////////////////////////

real Bankintermed::R_reg_Fktn(real, real)
{
	real buffer,buffer1;
...
real Bankintermed::R_c_Fktn(real, real)
{
	real buffer, buffer1;

	if(I/e <= temp2)
		R_c = (1+I_G)*y/I*(q_max+q_min)/2 - e/I*(1+R_reg)-1;
	else{
		buffer= 2*(q_max-q_min)*e/(y*q_max*q_max)*(1+R_reg)/(1+I_G);
		buffer1=sqrt(buffer);

		R_c = (1+I_G)*y*q_max/I*(1-buffer1)-1;
	}		
...
real Bankintermed::I_B_Fktn(real, real, real)
{
	real buffer1,buffer2;

	buffer1=I/y*(1+R_c)/q -1;
...
//
// Author:			Michael Meyer
// Last modified:	Wed Feb  7 15:39:45 MET 2001
// By:				Michael Meyer
//
///////////////////////////////////////////////////////////////////////////////

real Bankintermed::P_Fktn(real, real, real, real)
{
	real buffer;

	buffer = I_B-I_G;
	P = (nu-I_G)*I*(1+R_c)-(0.5*q*y*pow(buffer,2));
...
e=e_beta*e_old+e_alpha*q;
 ...
 S=(1-nu)*e*s;
	...
	R_c_Fktn(I_G,R_reg);
	I_B_Fktn(I_G,R_c,q);
	P_Fktn(I_G,I_B,R_c,q);

	G = P-(((nu-I_G)*I-df)*(1+R_reg));			// Systemdynamik
