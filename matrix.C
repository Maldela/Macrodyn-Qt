...
////////////////////////////////////////////////////////////////////////////////
matrixDef::matrixDef(const unsigned short& r,const unsigned short& c) {
	row=r;
	column=c;
...
////////////////////////////////////////////////////////////////////////////////
// Class name:      matrixFn                                                  //
// Member function: Log::log()Matrix                                                //
	...
	if(row==1) {
		...
		for(zz=0;zz<row;zz++) {
			...
			for(ss=0;ss<row;ss++) {
				...
				mi->m[ss][zz]=sign*(determinant(a)/detm);//[zz,ss]´=[ss,zz]
				sign*=(-1);
			}
		}
		delete a;
...
void matrixFn::multiply(const matrixDef* A, const matrixDef* B, matrixDef* C) {
	unsigned short z,s,i;
	unsigned short row = A->row;
	unsigned short ac = A->column;
	unsigned short column = B->column;
	for(z=0;z<row;z++)
		for(s=0;s<column;s++) {
			C->m[z][s]=0;
			for(i=0;i<ac;i++)
...
void matrixFn::multiplyATB(const matrixDef* A,const matrixDef* B,matrixDef* C){
	unsigned short z,s,i;
	unsigned short row = A->column; // A´
	unsigned short ac = A->row;  // A´
	unsigned short column = B->column;
	for(z=0;z<row;z++)
		for(s=0;s<column;s++) {
			C->m[z][s]=0;
...
//Log::log() << "ols constructor stream" << endl;
...
//Log::log() <<"varname="<<varname<<" y_num="<<y_num<<endl;
...
//Log::log() <<"varphi_var_num="<<varphi_var_num<<endl;
...
//Log::log() <<"varphi_numPtr["<<j<<"]="<<varphi_numPtr[j]<<endl;
//Log::log() <<"varphi_lagPtr["<<j<<"]="<<varphi_lagPtr[j]<<endl;
	}
//Log::log() <<"varphi_num="<<varphi_num<<endl;

	if(y_num<varphi_numPtr[0]) {
        Log::log() <<"length of forecast vector="<<y_num<<" < ";
        Log::log() <<varphi_numPtr[0]<<"=length of first input vector in varphi"<<endl;
        Log::log() <<"--> this will work, but are you qreally sure?"<<endl;
	}
	if(y_num>varphi_numPtr[0]) {
        Log::log() <<"length of forecast vector="<<y_num<<" > ";
        Log::log() <<varphi_numPtr[0]<<"=length of first input vector in varphi"<<endl;
        Log::log() <<"--> olsClass::updateY can not fill up Y"<<endl;
		exit(-1);
	}
...
// Log::log() << "VARPHI->m["<<ij<<"]="<< VARPHI->m[ij][0] << endl;
			ij++;
		}
...
//Log::log() << "ols estimate" << endl;
	matrix->multiplyABT(VARPHI,VARPHI,Rt); // P*VPY=THETA
	matrix->add(R,Rt,R);
	matrix->inverse(R,P);
//Log::log() <<"P:" << endl; matrix->Log::log()Matrix(P);

	// updateY
	updateY();

	matrix->multiplyABT(VARPHI,Y,VPYt); // VARPHI*Y'=VPYt
//Log::log() <<"VPYt:" << endl; matrix->Log::log()Matrix(VPYt);
...
//Log::log() <<"THETA:" << endl; matrix->Log::log()Matrix(THETA);
...
Log::log() << "ols forecast2 not implemented yet - exit" << endl;
exit(-1);
}
...
//Log::log() << "rls constructor stream" << endl;
	Pvarphi = NULL;	
	VtP = NULL;
	vPv = NULL;
	TEC = NULL;
...
//Log::log() << "rls init 1" << endl;
	olsClass::initialize();
//Log::log() << "rls init 2" << endl;
	int i,j;
...
// initialize scalar lambda
    ...
    //Log::log() <<"Pvarphi:" << endl; matrix->Log::log()Matrix(Pvarphi);
    ...
    //Log::log() <<"vPv:" << endl; matrix->Log::log()Matrix(vPv);
    //Log::log() <<"lambda=" << lambda<< endl;
...
// Log::log() <<"THETA:" << endl; matrix->Log::log()Matrix(THETA);
// Log::log() <<"VARPHI:" << endl; matrix->Log::log()Matrix(VARPHI);
// Log::log() <<"P:" << endl; matrix->Log::log()Matrix(P);
// Log::log() <<"Pvarphi:" << endl; matrix->Log::log()Matrix(Pvarphi);
// Log::log() <<"vPv:" << endl; matrix->Log::log()Matrix(vPv);
// Log::log() <<"lambda=" << lambda<< endl;
...
// Log::log() <<"YF:" << endl; matrix->Log::log()Matrix(YF);
// Log::log() <<"Y:" << endl; matrix->Log::log()Matrix(Y);
// Log::log() <<"FE" << endl; matrix->Log::log()Matrix(FE);
// Log::log() <<"lambda*Pvarphi:" << endl; matrix->Log::log()Matrix(Pvarphi);
// Log::log() <<"TEC:" << endl; matrix->Log::log()Matrix(TEC);
// Log::log() <<"THETA:" << endl; matrix->Log::log()Matrix(THETA);
...
// Log::log() <<"VtP:" << endl; matrix->Log::log()Matrix(VtP);
// Log::log() <<"PEC:" << endl; matrix->Log::log()Matrix(PEC);
// Log::log() <<"P:" << endl; matrix->Log::log()Matrix(P);
...
// Log::log() <<"vPv:" << endl; matrix->Log::log()Matrix(vPv);
// Log::log() <<"lambda=" << lambda<< endl;
// Log::log() <<"lambda*P=" << lambda*P->m[0][0]<< endl;
}
...
//Log::log() << "els constructor stream" << endl;
	TV = NULL;	
	upsilon = NULL;
...
// enlarge varphi_num for upsilon so that all other matricies can be initialized correctly
	if(upsilonLag>0) {
//Log::log() <<"elsClass:org varphi_num="<<varphi_num<<endl;
//Log::log() <<"elsClass:org varphi_var_num="<<varphi_var_num<<endl;
		varphi_numPtr[varphi_var_num]=y_num; // number of variables in upsilon vector			
		varphi_lagPtr[varphi_var_num]=upsilonLag;
//Log::log() <<"elsClass:varphi_numPtr["<<varphi_var_num<<"]="<<varphi_numPtr[varphi_var_num]<<endl;
//Log::log() <<"elsClass:varphi_lagPtr["<<varphi_var_num<<"]="<<varphi_lagPtr[varphi_var_num]<<endl;
		varphi_num+=(varphi_numPtr[varphi_var_num]  //sum (number_of_assets
			    *varphi_lagPtr[varphi_var_num]);// * time_lag_of_var_j)	
		varphi_var_num +=1;
	}
//Log::log() <<"elsClass:varphi_num="<<varphi_num<<endl;
//Log::log() <<"elsClass:varphi_var_num="<<varphi_var_num<<endl;
...
//for(j=0;j<y_num;j++) Log::log() <<"upsilon["<<j<<"]="<<upsilon[j];Log::log() << endl;
...
//Log::log() << "sg constructor stream" << endl;
	B = NULL;
	BI = NULL;
	ye = NULL;
	Kiid = NULL;
...
//Log::log() << "sg init 1" << endl;
	elsClass::initialize();
//Log::log() << "sg init 2" << endl;
...
//Log::log() <<"THETA:" << endl; matrix->Log::log()Matrix(THETA);
//for(int j=0;j<y_num;j++) Log::log() <<"upsilon["<<j<<"]="<<upsilon[j];Log::log() << endl;
//Log::log() <<"VARPHI:" << endl; matrix->Log::log()Matrix(VARPHI);
//Log::log() << "r=" << r << endl;
//Log::log() << "alpha=" << alpha << endl;
}
...
//Log::log() << "sg initializeUnbiasedForecast" << endl;
	int i,j,ij,ii,jj;
	...
	if((sigma==0)&&(epsilon==0))
        ...
        //Log::log() << "lagMax=" << lagMax << endl;

        qreal epsval=1.0/(4.0*(y_num+2)*lagMax);
		if(epsilon > epsval ) {
            Log::log()  <<"epsilon="<<epsilon<<" > "<< epsval << endl;
    ...
    //Log::log() <<"sgClass::initializeUnbiasedForecast VARPHI:" << endl; matrix->Log::log()Matrix(VARPHI);
...
//Log::log() <<"initunbF  THETA:" << endl; matrix->Log::log()Matrix(THETA);
}
...
//Log::log() << "sg unbiasedForecast" << endl;
/*
qreal R=1.01 ;
qreal a=0.65 ;
qreal beta=1.04 ;
...
//Log::log() <<"VARPHI:" << endl; matrix->Log::log()Matrix(VARPHI);
//Log::log() <<"yeSav:" << endl; matrix->Log::log()Matrix(YF);

	for(i=0;i<y_num;i++)
		for(j=0;j<y_num;j++)
			B->m[j][i]=THETA->m[i+ij][j]; // copy B1 from THETA into dummy B
//Log::log() <<"B:" << endl; matrix->Log::log()Matrix(B);

	matrix->inverse(B,BI); // BI=B^-1
//Log::log() <<"BI:" << endl; matrix->Log::log()Matrix(BI);
