...
// color alt      color = int (floor( ( double (h(k)) / double (h_max) ) * 34));
    ...
    QTextStream(&outFile) << char( color );
      if( screenGraphics ) {
        screenGraphics->setRect(dummy_x,dy,stepX,stepY,QColor(redFromUnit(hitpoint),greenFromUnit(hitpoint),blueFromUnit(hitpoint),255));
        //screenGraphics->setRect(dummy_x,dy,stepX,stepY,QColor(redFromUnit(hitpoint),greenFromUnit(hitpoint),blueFromUnit(hitpoint),(hitpoint*300<255?hitpoint*300:255)));
        //screenGraphics->setRect(dummy_x,dy,stepX,stepY,QColor(0,0,0,(hitpoint*300<255?hitpoint*300:255)));
