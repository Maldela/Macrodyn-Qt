...
if(*zParam<zmax)
	{
		...
		if( screenGraphics ) 
            screenGraphics->setRectangularBigPoint(*xParam,*yParam,col,2);
...
color_plot_b::color_plot_b(int p_size, baseModel* const cMod,const xyRange& axes,
          MacrodynGraphicsItem* const graph)
          :geometricJob(cMod,axes,graph)
{
    limit= length / 10;			// 10% are thrown away
...
(col==3?col=5:(col>=5?col=3:col=col)); //exchange green and black
        if( screenGraphics )
            screenGraphics->setRectangularBigPoint(*xParam,*yParam,col,2);
            //screenGraphics->setBigPoint(*xParam,*yParam,col,psize);
     }
