...
class muTimeSeriesPlot : public timeSeriesJob {
protected:
    QFile outFile;
    int multiplot_num;		// number of additional variables to plot
    qreal ** multiplotAdr;	// addresses of additional variables
    qreal * multiplotOld;	// to store old values
    qreal *zParam;
    qreal* multiplot_values;

public:
    muTimeSeriesPlot(baseModel* const, const QString&, MacrodynGraphicsItem* const=NULL,
        const QString& = "", int const=0, char** const=NULL, QString multiPlotLabel=NULL, qreal* ts_init_values=NULL);
    ~muTimeSeriesPlot();
    void simulation();
};
